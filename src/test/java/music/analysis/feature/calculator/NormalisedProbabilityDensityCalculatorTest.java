...
public class NormalisedProbabilityDensityCalculatorTest {

    private NormalisedProbabilityDensityCalculator fitnessCalculator = new NormalisedProbabilityDensityCalculator();
    ...
    public void testCalculateReward_givenWorstValue() throws Exception {
        ...
        when(featureMock.getExpectedValue()).thenReturn(0.0);
        when(featureMock.getFeatureResult()).thenReturn(1.0);
        when(featureMock.getStandardDeviation()).thenReturn(0.31);
        //when
        int reward = fitnessCalculator.calculateReward(featureMock);
        //then
