...
public class SimpleMutationTest {
    ...
    private SimpleMutation<Integer> mutation = new SimpleMutation<>(1.0, randomMock, values);
    ...
    public void testMutateChromosome() throws Exception {
        ...
        assertThat(chromosomes.get(0).getGenes(), contains(4, 4, 9, 2, 6, 1, 3, 5));
        assertThat(chromosomes.get(1).getGenes(), contains(9, 8, 8, 3, 6, 7, 8, 6));
    }

    private List<Chromosome<Integer>> prepareData() {
        Chromosome<Integer> firstChromosome = new Chromosome<>(Arrays.asList(4, 4, 0, 2, 6, 1, 3, 5));
        Chromosome<Integer> secondChromosome = new Chromosome<>(Arrays.asList(9, 8, 8, 10, 6, 7, 8, 6));
